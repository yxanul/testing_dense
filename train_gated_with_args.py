...
Supports command-line arguments for easy experimentation.

Example usage:
    # Train with gated attention (default)
    python train_gated_with_args.py --use_gated_attention

    # Train baseline without gated attention
    python train_gated_with_args.py --no_gated_attention

    # Custom configuration
    python train_gated_with_args.py --batch_size 16 --learning_rate 4e-4 --use_fp8
"""
import os
import time
import math
...
class TrainingConfig:
    ...
    # WandB
    use_wandb: bool = True
    wandb_project: str = "gpt2-experiments"
    ...
    def __post_init__(self):
        ...
        if self.use_gated_attention:
            self.model_name = f"{self.model_name}-gated"
        else:
            self.model_name = f"{self.model_name}-baseline"

        # Update wandb tags
        if not self.wandb_tags:
            self.wandb_tags = []
        if self.use_gated_attention:
            self.wandb_tags.append("gated-attention")
        else:
            self.wandb_tags.append("baseline")
        if self.use_fp8:
            self.wandb_tags.append("fp8")
        if self.use_rope:
            self.wandb_tags.append("rope")
        self.wandb_tags.append("rtx5090")
...
def parse_args():
    ...
    parser.add_argument("--wandb_project", type=str, default="gpt2-experiments",
...
def train_step(model, batch, optimizer, scheduler, scaler, config):
    ...
    # FP8 is handled inside the model
...
def save_checkpoint(model, optimizer, scheduler, step, config, metrics=None):
    ...
    run_dir = os.path.join(config.checkpoint_dir, config.model_name)
    os.makedirs(run_dir, exist_ok=True)

    checkpoint_path = os.path.join(run_dir, f"checkpoint-{step}.pt")
...
def main():
    ...
    if config.use_gated_attention:
        print("✓ Gated attention enabled")
    else:
        print("✗ Baseline model (no gated attention)")

    if config.use_fp8:
        print("✓ FP8 compute enabled (E4M3 fwd, E5M2 bwd)")
    else:
        print("✗ FP8 disabled")
    ...
    eval_dataloader = create_dataloader(config, tokenizer)

    # Training loop
    print(f"\nStarting training for {config.num_train_steps} steps...")
    print(f"Effective batch size: {config.batch_size * config.gradient_accumulation_steps}")
    print(f"Mixed precision: {config.mixed_precision} + FP8: {config.use_fp8}")
    ...
    for step in range(start_step, config.num_train_steps):
        ...
        if (step + 1) % config.log_interval == 0:
            ...
            print(f"Step {step+1}/{config.num_train_steps} | "
                  ...
                  f"Model: {'Gated' if config.use_gated_attention else 'Baseline'}")
    ...
    print("\nTraining completed!")
    print(f"Model type: {'Gated Attention' if config.use_gated_attention else 'Baseline'}")
